...
async def echo(message: types.Message):
    if message.text == 'Лекційний':
        ...
        await bot.send_message(message.from_user.id, '🪟Меню', reply_markup = nav.lecMenu)
    elif message.text == '📜Меню':
        await bot.send_message(message.from_user.id, '📜Меню', reply_markup = nav.practMenu)
    elif message.text == '👈👈👈':
...
#--- Практичний тиждень
    if message.text == 'Практичний':
        await bot.send_message(message.from_user.id, 'Практичний', reply_markup = nav.practMenu)
    elif message.text == '🐻Ймовірність':
